...
数据文件：`out/FB15k-237/example.json`（每行一个query对象）
...
```json
{
  ...
  "candidates": [
    {
      "name": "entity_id",
      "rules": [ruleId1, ruleId2, ...],      // 包含的规则ID
      "ruleSurprisal": [1.0, 0.8,...],    // 上述每个rule对应的surprisal（原始）
      "newRuleSurprisal": [1.8, 0, ...],    // 被抑制置为0，被添加则加对应的值
      "GT": true/false,         // 是否为Ground Truth
      "positiveDep": 10,
      "negativeDep": 6,
      "originalSurprisal": 1.5,  // ruleSurprisal之和
      "newSurprisal": 2.0    // 最终的累积Surprisal，用于评分
    }
  ],
  "ruleInfo": {"bodySize": 50, "headSize":30, ...},
  "depInfo": {"id1": 50, "id2": 100, "bodySize": 20, ...} // 默认id1<id2
}
```
